...
!
    ! THIS HAVE BEEN ADDED BY A. GODARD PALLUET TO REDUCE THE BASIS WITH EMAX
    ! EMAX IS NOW APPLICABLE TO THE GLOBAL BASIS AND NOT ONLY THE COLLIDER
    if ((etemp(i1) + e2rot(i2)).gt.emax) then
      print*, ' EMAX', emax, (etemp(i1) + e2rot(i2))
      go to 170
    endif
    ! END OF MODIFICATION - THIS CONDITION MUST BE COMMENTED TO GO BACK TO EMAX ONLY TRUNCATES COLLIDER BASIS

    print*, etemp(i1), etemp(i1)
    nlevel = nlevel + 1
...
130   format(/2x,'ASYMMETRIC TOP + LINEAR MOLECULE LEVELS'/ &
  ...
  do i = 1, nlevel
    ...
    write (6, 135) i, j1, ishold(i), kp(i), ko(i), j2, ecm, &
      ...
      (i - 1)*narray + isizh(i))
    write (9, 135) i, j1, ishold(i), kp(i), ko(i), j2, ecm, &           ! THIS IS MISSING IN THE ONLINE VERSION OF THE CODE
      (chold(nn),nn=(i - 1)*narray + 1, &
      (i - 1)*narray + isizh(i))
